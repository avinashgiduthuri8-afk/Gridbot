...
def build_newgrid_conversation(app_context: "BotAppContext") -> ConversationHandler:  # noqa: F821
    ...
    # ------------------------------------------------------------------
    # Fallback: any other command sent mid-conversation
    # ------------------------------------------------------------------
    # Without this, a state that only registers a CallbackQueryHandler (e.g.
    # GRID_SETUP_MODE, SELECT_COIN, TRAILING_CHOICE, SELECT_MODE, CONFIRM)
    # has no way to recover if the user types anything instead of tapping
    # the expected button. PTB tracks conversation state per (chat, user);
    # once wedged there, entry_points are no longer checked, so /newgrid
    # stops responding entirely until the process restarts. This handler
    # gives every state an escape hatch: any stray command ends the
    # in-progress setup (with a nudge back to /newgrid) instead of hanging
    # silently. Runs after the more specific /cancel and /newgrid fallbacks
    # below since ConversationHandler checks fallbacks in list order.
    async def unexpected_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        await update.message.reply_text(
            "⚠️ Grid setup cancelled — that command isn't part of this flow.\n\n"
            "Use /newgrid to start over."
        )
        return ConversationHandler.END

    async def conversation_timed_out(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        # `update` may be a stale Update or None depending on how the timeout
        # fired; only try to notify the user if we actually have a chat to
        # message. Failure to notify must never block the state from
        # clearing — that would recreate the exact wedge this fix removes.
        chat = getattr(update, "effective_chat", None) if update is not None else None
        if chat is not None:
            try:
                await context.bot.send_message(
                    chat_id=chat.id,
                    text="⌛ Grid setup timed out after 5 minutes of inactivity.\n\n"
                         "Use /newgrid to start over.",
                )
            except Exception:  # noqa: BLE001
                log.warning("Could not notify user of newgrid conversation timeout", exc_info=True)
        return ConversationHandler.END
    ...
    return ConversationHandler(
        ...
        states={
            ...
            # TypeHandler (not MessageHandler) because the update that
            # re-triggers on timeout may be the last CallbackQuery received
            # (e.g. GRID_SETUP_MODE, SELECT_MODE, CONFIRM are button-only
            # states) rather than a Message — a MessageHandler filter would
            # silently fail to match and the timeout would never fire.
            ConversationHandler.TIMEOUT: [TypeHandler(Update, conversation_timed_out)],
        },
        fallbacks=[
            MessageHandler(filters.Regex(r"^/cancel$"), cancel),
            # Re-entrant: /newgrid can restart the flow from any state, not
            # just when no conversation is active.
            MessageHandler(filters.Regex(r"^/newgrid$"), start),
            # Catch-all: any other command bails out cleanly instead of
            # wedging the conversation (see unexpected_command docstring).
