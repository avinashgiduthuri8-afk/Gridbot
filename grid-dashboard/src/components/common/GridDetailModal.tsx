...
export const GridDetailModal: React.FC<GridDetailModalProps> = ({
  ...
  const [manualTradeSide, setManualTradeSide] = useState<'buy' | 'sell' | null>(null);
  const [actionLoading, setActionLoading] = useState(false);
  const [actionError, setActionError] = useState<string | null>(null);

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && !manualTradeSide) {
        onClose();
      }
    };
    if (grid) {
  ...
  }, [grid, onClose, manualTradeSide]);

  if (!grid) return null;

  const isActive = grid.status === 'active';
  const isPaused = grid.status === 'paused';
  ...
  const handlePauseResume = async () => {
    ...
    try {
      if (isActive) {
        await api.pauseGrid(grid.grid_id);
      } else {
        await api.resumeGrid(grid.grid_id);
      }
      onRefresh?.();
      onClose();
    } catch (err: any) {
  ...
  const handleStop = async () => {
    if (!window.confirm(`Are you sure you want to stop grid ${grid.grid_id}?`)) {
      return;
    }
    setActionLoading(true);
    setActionError(null);
    try {
      await api.stopGrid(grid.grid_id);
      onRefresh?.();
      onClose();
    } catch (err: any) {
      setActionError(err.message || 'Stop grid failed');
    } finally {
      setActionLoading(false);
    }
  };
    ...
    <>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
              <div>
                ...
                <div style={{ fontSize: '0.75rem', color: 'var(--text-dark)', fontFamily: 'monospace', marginTop: '0.2rem' }}>
                  ID: {grid.grid_id}
                </div>
              </div>
            </div>
              ...
              {(isActive || isPaused) && (
                <>
                  <button
                    ...
                    onClick={() => setManualTradeSide('buy')}
                    style={{ backgroundColor: 'rgba(16, 185, 129, 0.15)', color: 'var(--success)', borderColor: 'var(--success)' }}
                  >
                  ...
                  <button
                    className="action-btn"
                    onClick={() => setManualTradeSide('sell')}
                    style={{ backgroundColor: 'rgba(6, 182, 212, 0.15)', color: 'var(--accent-cyan)', borderColor: 'var(--accent-cyan)' }}
                  >
      ...
      {manualTradeSide && (
        <ManualTradeModal
          ...
          isOpen={true}
          onClose={() => setManualTradeSide(null)}
          onSuccess={() => {
            setManualTradeSide(null);
            onRefresh?.();
          }}
        />
      )}
