...
class DCAManager:
    ...
    async def handle_order_filled(
        ...
        Acquires the per-grid lock itself — callers must NOT already be
        holding it. If you're calling this from code that runs inside
        `async with self._grid_lock(grid_id):` (e.g. from within
        check_grid_triggers()'s call chain), call
        `_handle_order_filled_locked()` directly instead, or this will
        deadlock on asyncio.Lock's non-reentrancy. See _execute_stop_loss for
        the one internal case that needs this.
        """
        ...
        async with self._grid_lock(grid_id):
            await self._handle_order_filled_locked(order_id, fill_price, fill_qty)

    async def _handle_order_filled_locked(
        self, order_id: str, fill_price: float, fill_qty: float
    ) -> None:
        """Inner implementation of handle_order_filled — must be called while
        already holding _grid_lock(order['grid_id']).

        Split out so _execute_stop_loss can process a synchronous
        (immediate) exchange fill without re-acquiring a lock it already
        holds via check_grid_triggers() — asyncio.Lock is not reentrant, so
        calling the public handle_order_filled() from there would deadlock
        the whole PriceMonitor tick loop (it awaits check_grid_triggers()
        sequentially per grid).
        """
        order = await self._repos.orders.get(order_id)
        if not order:
            log.warning("handle_order_filled called for unknown order %s", order_id)
    ...
    async def _execute_stop_loss(self, grid: dict, current_price: float) -> None:
        ...
        if getattr(order, "status", None) == OrderStatus.FILLED.value:
            ...
            # Mark STOPPING before applying the fill, mirroring the async
            # branch below — _on_sell_filled only finalizes to STOPPED when
            # it observes the grid already in STOPPING. Without this, a
            # synchronous fill leaves the grid ACTIVE at fill time and
            # _on_sell_filled treats it as a normal full-exit cycle,
            # mis-recording a loss-cutting stop-loss as "completed".
            await self._repos.grids.update_state(
                grid_id,
                status=GridStatus.STOPPING.value,
            )
            try:
                # _handle_order_filled_locked, not handle_order_filled: we're
                # already inside _grid_lock(grid_id) here (acquired by
                # check_grid_triggers -> _check_grid_triggers_locked ->
                # _execute_stop_loss), and asyncio.Lock is not reentrant —
                # calling the public, lock-acquiring handle_order_filled()
                # from here would deadlock this coroutine forever, which in
                # production freezes PriceMonitor's entire tick loop (it
                # awaits check_grid_triggers() sequentially per grid).
